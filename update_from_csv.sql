...
WHERE documento_norm IS NOT NULL AND LENGTH(documento_norm)=14 AND (documento_raiz IS NULL OR TRIM(documento_raiz)='');

-- Temp map
DROP TABLE IF EXISTS temp.tmp_map;
CREATE TEMP TABLE tmp_map(documento TEXT PRIMARY KEY, email TEXT, contato TEXT);
...
INSERT OR REPLACE INTO tmp_map VALUES ('17654804000107','auditorios@medgrupo.com.br','(21) 9 9891-0910 / (21) 98209-3040'); -- MEDGRUPO
...
-- Aplicar email quando vazio ou placeholder
UPDATE Clientes_Eventos AS c
SET email = COALESCE(
  (SELECT m.email FROM tmp_map m
...
-- Aplicar contato quando vazio
UPDATE Clientes_Eventos AS c
SET nome_responsavel = COALESCE(
  (SELECT m.contato FROM tmp_map m
    WHERE m.documento = COALESCE(c.documento_norm, REPLACE(REPLACE(REPLACE(c.documento,'.',''),'/',''),'-',''))
...
.mode column
.print ''
.print '== Restantes com email placeholder =='
