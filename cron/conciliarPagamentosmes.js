...
async function conciliarPagamentosD1() {
 ...
 // Bloco Corrigido: Mês Atual (do dia 1 até hoje)
  const hoje = new Date();
  const dataIniObj = new Date(hoje.getFullYear(), hoje.getMonth(), 1); // Define a data para o primeiro dia do mês corrente
  const dataFimObj = hoje; // Usa a data atual como data final
  
  // Formata as datas para o serviço por-data-arrecadacao
  const dataIni = ymd(dataIniObj); // 'YYYY-MM-DD' do primeiro dia
  const dataFim = ymd(dataFimObj); // 'YYYY-MM-DD' de hoje
  
  // Formata as datas para o serviço por-data-inclusao
  const dtIni = toDateTimeISO(dataIniObj, 0, 0, 0);   // Data e hora do início do primeiro dia do mês
  const dtFim = toDateTimeISO(dataFimObj, 23, 59, 59); // Data e hora do fim do dia de hoje
