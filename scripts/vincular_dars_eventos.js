 ...
 *
 * Agora com detecção de "PAGO" na célula da parcela:
 *   - Se a célula tiver "PAGO 1.234,56" (ou "1.234,56 PAGO"), a parcela é criada já como Pago.
 *   - Define data_pagamento por padrão como a data de vencimento daquela parcela,
 *     podendo ser alterado com --pag-data=hoje.
 *
 * Uso:
 *   npm i xlsx
 ...
 *   --dry-run     não altera o banco; exibe o que faria
...
function brMoneyToFloat(v) {
  ...
  let s = String(v).trim();
  if (!s) return NaN;
  // Extrai o primeiro número "1.234,56" presente na string
...
function parsePagoFlag(cellValue) {
  // Considera "pago" se houver a palavra, ignorando caixa e espaços
  const txt = String(cellValue || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  return /pago/i.test(txt);
}
function parseDatePtBR(s) {
  if (!s) return null;
  const txt = String(s).trim();
  const fmts = [
    { re: /^(\d{1,2})\/(\d{1,2})\/(\d{2,4})$/, map: (d,m,y) => [y.length===2?('20'+y):y, m.padStart(2,'0'), d.padStart(2,'0')].join('-') },
    { re: /^(\d{4})-(\d{2})-(\d{2})(?:\s+\d{2}:\d{2}:\d{2})?$/, map: (y,m,d) => [y,m,d].join('-') },
  ];
  for (const f of fmts) {
    const m = txt.match(f.re);
    if (m) return f.map(m[1], m[2], m[3]);
  }
  // Fallback simples (dayfirst)
...
(async () => {
  ...
  // Mapeia colunas por nome (tolerante a espaços e variações)
  ...
  for (const r of rows) {
    ...
    const ev = await get(`SELECT id, id_cliente, status FROM Eventos WHERE numero_processo = ?`, [numProcesso]);
    if (!ev) { console.warn('[NÃO ENCONTRADO] Evento não localizado:', numProcesso, '-', empresa); notFound++; continue; }
    ...
    for (const p of parcelas) {
      ...
      if (existing?.dar_id) {
        // Se já existe, mas queremos marcar como pago e ainda está pendente, atualiza
