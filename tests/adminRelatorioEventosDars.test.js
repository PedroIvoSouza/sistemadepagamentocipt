...
test('relatorio de dars de eventos filtra por data', async () => {
  ...
  await run(
    `INSERT INTO dars (id, status, numero_documento, data_vencimento, valor) VALUES (1,'Emitido','DAR123','2025-08-15',100);`
  );
  await run(`INSERT INTO DARs_Eventos (id_dar, id_evento) VALUES (1,1);`);
  await run(
    `INSERT INTO dars (id, status, numero_documento, data_vencimento, valor) VALUES (2,'Emitido','DAR999','2025-09-10',50);`
  );
  ...
  assert.ok(!/10\/09\/2025/.test(parsed.text));
