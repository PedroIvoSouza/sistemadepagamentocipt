// tests/darsRoutesPreviewVencido.test.js
...
test('preview indica DAR vencido', async () => {
  const dbPath = path.resolve(__dirname, 'test-dars-preview-vencido.db');
  ...
  require.cache[cobrancaPath] = { exports: { calcularEncargosAtraso: async dar => ({ valorAtualizado: dar.valor + 20, novaDataVencimento: '2024-01-10' }) } };
  ...
  const res = await supertest(app).get('/30/preview').expect(200);
  assert.equal(res.body.darVencido, true);
  assert.equal(res.body.calculo.valorAtualizado, 220);
});
