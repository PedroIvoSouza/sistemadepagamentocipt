...
async function buildPayloadFromEvento(eventoId) {
  ...
  return {
    ...
    // crús adicionais p/ filename
    _raw: ev
  };
}

// ---------- desenho (cabeçalho/rodapé) ----------
function drawHeader(doc, payload) {
  const save = { x: doc.x, y: doc.y };
  const larguraUtil = doc.page.width - doc.page.margins.left - doc.page.margins.right;
  const left = doc.page.margins.left;
  doc.save();
  doc.fillColor('#000').font('Times-Bold').fontSize(12);
  // Cabeçalho em 3 linhas, centralizado e caixa alta
  doc.text(String(payload.org_uf || '').toUpperCase(), left, cm(2.5) - 12, { width: larguraUtil, align: 'center' });
  doc.text(String(payload.org_secretaria || '').toUpperCase(), left, doc.y, { width: larguraUtil, align: 'center' });
  doc.text(String(payload.org_unidade || '').toUpperCase(), left, doc.y, { width: larguraUtil, align: 'center' });

  // volta cursor para a área útil
  doc.restore();
  doc.x = save.x; doc.y = Math.max(save.y, doc.page.margins.top + cm(1));
}
...
function drawFooter(doc) {
  ...
  const larguraUtil = doc.page.width - doc.page.margins.left - doc.page.margins.right;
  const left = doc.page.margins.left;
  const bottomY = doc.page.height - doc.page.margins.bottom + 6; // um pouco acima da margem
  doc.save();
  doc.font('Times-Roman').fontSize(9).fillColor('#333');
  const pageNum = doc.page.pageNumber || 1;
  doc.text(`Página ${pageNum}`, left, bottomY, { width: larguraUtil, align: 'right' });
  doc.restore();
  doc.x = save.x; doc.y = save.y;
}
...
function textJustify(doc, str, opts={}) {
  ...
  const left = doc.page.margins.left;
  setBodyFont(doc);
  doc.text(str, left + (opts.indentLeft || 0), doc.y, {
    width: (opts.width || (larguraUtil - (opts.indentLeft || 0))),
    align: 'justify',
    lineGap: 2
  });
...
function headingClausula(doc, titulo) {
  doc.moveDown(0.6);
  doc.font('Times-Bold').fontSize(12).fillColor('#000').text(titulo.toUpperCase(), {
    align: 'left'
  });
  doc.moveDown(0.2);
  setBodyFont(doc);
...
function drawDiscriminacaoTable(doc, payload) {
  ...
  const cols = [
    { key: 'desc',   label: 'Discriminação/Área utilizada', w: larguraUtil * 0.40 },
    { key: 'area',   label: 'Área (m²)/Capacidade',         w: larguraUtil * 0.23 },
    { key: 'dias',   label: 'Nº de dias',                   w: larguraUtil * 0.15 },
    { key: 'valor',  label: 'Valor total',                  w: larguraUtil * 0.22 },
  ];
  const rowH = 20;

  const drawRow = (cells, yy, header=false) => {
    let x = left;
    if (header) {
      doc.font('Times-Bold');
    } else {
    ...
    cells.forEach((cell, i) => {
      doc.text(String(cell||''), x + 4, yy + 4, { width: cols[i].w - 8, align: header ? 'left' : (i===2?'center': (i===3?'right':'left')), lineBreak:false });
  ...
  // Header
  let y = doc.y + 4;
  if (y + rowH > doc.page.height - doc.page.margins.bottom - 10) { doc.addPage(); y = doc.page.margins.top; }
  drawRow(cols.map(c=>c.label), y, true); y += rowH;

  const desc = `${payload.local_espaco} do ${payload.imovel_nome}\nRealização: ${payload.data_evento || '-'}\nMontagem: ${payload.data_montagem || '-'}\nDesmontagem: ${payload.data_desmontagem || '-'}`;
  const area = `${payload.area_m2_fmt} (capacidade para ${payload.capacidade_pessoas} pessoas)`;
  const dias = String(payload.numero_dias || 1);
  const valor = payload.valor_total_fmt;

  if (y + rowH > doc.page.height - doc.page.margins.bottom - 10) { doc.addPage(); y = doc.page.margins.top; drawRow(cols.map(c=>c.label), y, true); y += rowH; }
  drawRow([desc, area, dias, valor], y, false);
  y += rowH;
...
async function gerarTermoEventoPdfEIndexar(eventoId, { returnBuffer=false } = {}) {
  ...
  const doc = new PDFDocument({
    size: 'A4',
    margins: abntMargins(0, 0), // 2.5cm padrão + 0,5 topo/rodapé já vem do helper
  });
  ...
  // ======= CONTEÚDO =======
  // Header + Footer da primeira página
  drawHeader(doc, payload);
  drawFooter(doc);
  ...
  // Título grande (em bloco justificado com recuo de 6cm)
  {
    const titulo = `TERMO DE PERMISSÃO DE USO QUE CELEBRAM ENTRE SI DE UM LADO A ${payload.permitente_razao} E DO OUTRO ${payload.permissionario_razao}.`;
    textJustify(doc, titulo, { indentLeft: cm(6), spaceAfter: 0.8 });
  }

  // Processo / Termo
  doc.font('Times-Roman').fontSize(12);
  doc.text(`Processo n°: ${payload.processo_numero || '-'}`, left, doc.y, { width: larguraUtil, align: 'left' });
  doc.text(`Termo n°: ${payload.termo_numero || '-'}`, left, doc.y, { width: larguraUtil, align: 'left' });
  doc.moveDown(0.6);

  // PERMITENTE
  {
    const t = `PERMITENTE: ${payload.permitente_razao}, inscrita no CNPJ/MF sob o nº ${payload.permitente_cnpj} e estabelecido(a) no(a) ${payload.permitente_endereco}, de acordo com a representação legal que lhe é outorgada por portaria e representado pelo responsável: ${payload.permitente_representante_cargo}, Sr. ${payload.permitente_representante_nome}, inscrito no CPF sob o nº ${payload.permitente_representante_cpf}.`;
    textJustify(doc, t, { spaceAfter: 0.4 });
  }

  // PERMISSIONÁRIO(A)
  {
    const t = `PERMISSIONÁRIO(A): ${payload.permissionario_razao}, inscrito(a) no CNPJ/MF sob o nº ${payload.permissionario_cnpj} e estabelecido(a) em ${payload.permissionario_endereco}, representado por ${payload.permissionario_representante_nome || '(nome do responsável)'}, CPF ${payload.permissionario_representante_cpf || '(CPF)'}.`;
    textJustify(doc, t, { spaceAfter: 0.6 });
  }

  // CLÁUSULA PRIMEIRA
  headingClausula(doc, 'Cláusula Primeira – Do Objeto');
  {
    const t = `1.1 - O presente instrumento tem como objeto o uso pelo(a) PERMISSIONÁRIO(A) de área do ${payload.local_espaco} do imóvel denominado ${payload.imovel_nome}, para realização da “${payload.evento_titulo}”, a ser realizada em ${payload.data_evento || '-'}, das ${payload.hora_inicio} às ${payload.hora_fim}, devendo a montagem ser realizada no mesmo dia do evento e a desmontagem ao final, conforme proposta em anexo, estando disponível o uso do seguinte espaço:`;
    textJustify(doc, t, { spaceAfter: 0.4 });
  }
  ...
  textJustify(doc, `2.1 - O prazo de vigência se inicia na data de assinatura do presente termo até ${payload.vigencia_fim_datahora}.`, { spaceAfter: 0.4 });

  // CLÁUSULA TERCEIRA
  headingClausula(doc, 'Cláusula Terceira – Do Pagamento');
  {
    const t = [
      `3.1 - O(A) PERMISSIONÁRIO(A) pagará pela utilização do espaço o valor total de ${payload.valor_total_fmt} (${payload.valor_total_fmt.replace('R$','R$ ')}) através de Documento de Arrecadação – DAR, efetuado em favor da conta do Fundo Estadual de Desenvolvimento Científico, Tecnológico e de Educação Superior (${payload.fundo_nome}), devendo ser pago o valor de 50% até ${payload.pagto_sinal_data} e o restante até ${payload.pagto_saldo_data}.`,
  ...
  textJustify(doc, `9.1 - As questões decorrentes da execução deste Instrumento que não possam ser dirimidas administrativamente serão processadas e julgadas no Foro da Cidade de Maceió – AL, que prevalecerá sobre qualquer outro, por mais privilegiado que seja, para dirimir quaisquer dúvidas oriundas do presente Termo.`, { spaceAfter: 0.8 });

  // Local/data
  setBodyFont(doc);
  doc.text(`${payload.cidade_uf}, ${payload.data_assinatura}.`, left, doc.y, { width: larguraUtil, align: 'left' });
  doc.moveDown(3);

  // Linhas de assinatura
  const signLine = (label) => {
    const w = larguraUtil * 0.9;
    const x = left + (larguraUtil - w)/2;
    const y = doc.y;
