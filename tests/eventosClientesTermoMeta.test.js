...
test('GET /api/portal/eventos/:id/termo/meta uses existing signed url without calling assinafy', async () => {
  const pdfPath = path.resolve(__dirname, 'fake.pdf');
  fs.writeFileSync(pdfPath, 'pdf');
  ...
  class FakeDB {
    constructor() {
      this.doc = {
        ...
        pdf_public_url: 'public.pdf',
        assinafy_id: 'ASS1',
        signed_pdf_public_url: 'https://signed.example.com/file.pdf',
        status: 'assinado',
  ...
  await request
    ...
    .expect(res => {
      assert.equal(res.body.signed_pdf_public_url, 'https://signed.example.com/file.pdf');
      assert.equal(res.body.status, 'assinado');
      assert.equal(res.body.signed_at, '2024-01-01T00:00:00.000Z');
    });
